...
{
  dwide <- data.frame.wide(d, c("Case", "ChanceAnd", "ChanceSplit", "Inconsistent"), 
                           measure, "Strategy")
  ...
  dsubm <- melt(dsub, c("Case", "ChanceAnd", "ChanceSplit", "Inconsistent"))
  print(head(dwide))
  ...
  ggplot(dsubm) + geom_boxplot(aes(x=variable, y=value)) + # , fill=factor(Inconsistent)
    ...
    scale_x_discrete("Strategy") + scale_y_continuous(measure) + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
}

d <- read.csv("paragon-random-abduce.csv")
#analyze.measure(d, "ExplainedPct", "rand_rand")
analyze.measure(d, "ChangePct", "rand_rand")
